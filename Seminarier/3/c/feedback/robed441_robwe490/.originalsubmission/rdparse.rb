...
##############################################################################
#
# This part defines the LogExp language
#
##############################################################################

class LogExp

  attr_accessor :logexParser

  def initialize
    $variables = Hash.new
    $variables.default = false
  
    assignmentNode = Struct.new(:a, :b){ def eval; $variables[a.a] = b.eval end}
    andNode = Struct.new(:a, :b){ def eval; a.eval && b.eval end}
    orNode = Struct.new(:a, :b){ def eval; a.eval || b.eval end}
    notNode = Struct.new(:a){ def eval; !a.eval end}
    boolNode = Struct.new(:a){ def eval; a end}
    varNode = Struct.new(:a){ def eval; $variables[a] end}

    @logexParser = Parser.new("logical expression parser") do
      token(/\s+/)
      token(/\(|\)|\w+/) {|m| m }
      ...
      rule :assign do
        match('(', 'set', :var, :expr, ')'){|_,_,a,b,_| assignmentNode.new(a, b)}
      end

      rule :expr do
        match('(', 'or', :expr, :expr, ')'){|_,_,a,b,_| orNode.new(a, b) }
        match('(', 'and', :expr, :expr, ')'){|_,_,a,b,_| andNode.new(a, b) }
        match('(', 'not', :expr, ')'){|_,_,a,_| notNode.new(a) }
        match(:term)
      end
  ...
  def evaluate
    ...
    if done(str) then
      ...
      root_node = @logexParser.parse str
      puts "=> #{root_node.eval}"
      evaluate
    end
  end
  ...
  def log(state = true)
    if state
      @logexParser.logger.level = Logger::DEBUG
    else
      @logexParser.logger.level = Logger::WARN
    end
  end
end
