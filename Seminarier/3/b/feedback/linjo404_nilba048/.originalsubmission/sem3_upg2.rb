...
##############################################################################
#
# This part defines the logic language
#
##############################################################################
...
class Logic
  ...
  def initialize   
    @logic_parser = Parser.new("logic evaluator") do
      token(/\s+/)
      token(/(\(|\)|\w+)/) {|m| m }
      token(/./)

      # save variable (name => value) pairs
      vars = Hash.new
      ...
      rule :assign do
        match('(', 'set', :var, :expr, ')') {|_, _, var, expr, _| vars[var] = expr}
      end
      ...
      rule :term do 
        match('true') {true}
        match('false') {false}
        # return value of variable if it has one saved, otherwise return variable name
        match(:var) {|var| vars[var] != nil ? vars[var] : var} 
      end
  ...
  def eval
    print "[logic evaluator] "
    str = gets
    if done(str) then
      return @result
    else
      puts "=> #{@result = @logic_parser.parse str}"
      eval
    end
  end
