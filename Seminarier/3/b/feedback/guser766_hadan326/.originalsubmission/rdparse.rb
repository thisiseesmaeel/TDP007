...
##############################################################################
#
# This part defines the language
#
##############################################################################

class Language
  
  attr_accessor :languageParser

  def initialize
    @languageParser = Parser.new("Parseruppgift") do

      # behållare med alla variabler och deras värde
      @all_variables = {}

      # För att kunna matcha med hela ord används regex för att convertera hela ord till en sträng
      token(/\w+/) { |m| m.to_s}
      token(/\s+/)
      token(/./) {|m| m }
      ...
      rule :ASSIGN do
        # Om variabeln "a" redan finns i @all_variables skrivs värdet över med "b", annars skapas ett nytt item med nyckeln "a" och värdet b
        match('(', 'set', :VAR, :EXPR, ')') {|_, _, a, b, _| @all_variables[a] = b} 
        match(:EXPR)
      end
      rule :EXPR do
      ...
      rule :TERM do
        match('true'){true}
        match('false'){false}
        # Om variabelns värde är en annan variabel kollas istället värdet på indexet av variabeln (a)
        match(:VAR) { |a| @all_variables[a] }
      end
      rule :VAR do
        match(String)
     end
