...
class Logic
  ...
  def initialize
    ...
    @logicParser = Parser.new("logic counter") do
      @variables = Hash.new
      token(/\w+/) {|m| m }
      token(/\s+/)
      token(/./) {|m| m }
      ...
      # Fråga: Kan inte skriva över :VAR till något annat?
      rule :ASSIGN do
        match( '(', 'set', :VAR, :EXPR, ')' ) { |_,_,a,b,_| @variables[a] = b }
        #match( '(', 'set', :EXPR, :EXPR, ')' ) { |_,_,a,b,_| @variables[a] = b }
      end
      ...
      # Kanske bör utökas?
      rule :VAR do
        match( '$', String ) {|_,a| @variables[a] }        
        match( String ) { |s| s }
      end
    
    

# match(:expr, '+', :bit) {|a, _, b| [a + b, 1].min }
      #   match('-', :bit) do |_,a|
      #     if a >= 1
      #       0
      #     else
  ...
  def done(str)
    if str == nil then
      ...
      return ["quit","exit","bye",""].include?(str.chomp)
    end
  end
  
  def count(iostream = $stdin)
    print "[logic counter] "
    str = iostream.gets
    if done(str) then
      puts "Bye."
...
# Examples of use
puts "\nwrite Logic.new.count to start...\n\n"
#Logic.new.count
