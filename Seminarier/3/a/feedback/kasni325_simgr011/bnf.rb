...
class Logical
  ...
  def initialize
    @logicalParser = Parser.new("logical expression") do
  ...
  def roll
    ...
    if done(str) then
      ...
      puts "=> #{@logicalParser.parse str}"
      roll
    end
  end

  def test(str)
    @logicalParser.parse str
  end
  
  def log(state = true)
    if state
      @logicalParser.logger.level = Logger::DEBUG
    else
      @logicalParser.logger.level = Logger::WARN
    end
  end
