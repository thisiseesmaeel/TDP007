...
class Lang
  ...
  def initialize
    ...
    rule :assign do
      match('(', 'set', :var, :expr, ')') {|_, _, a, b, _| @hash[a]=b }
    end
  ...
  def log(state = true)
    if state
      @lang.logger.level = Logger::DEBUG
    else
      @lang.logger.level = Logger::WARN
    end
  end
end
end
