...
#Extend class ArithmeticConstraint in order to be able to force subtraction or division
...
class ConstraintParser < Parser
    ...
    @parser=super("constraints") do
      ...
      rule :expr do
        ...
        match(:expr, '-', :term) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'-',b)
          # a-b=c <=> a=b+c
          Adder.new(conn_a,conn_b,conn_c).inverse_operator      #force inverse   
        end
      ...
      rule :term do
        ...
        match(:term, '/', :atom) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'/',b)
          # a/b=c <=> b*c=a
          Multiplier.new(conn_a,conn_b,conn_c).inverse_operator      #force inverse
  ...
  def get_connector(var)
    return var if var.is_a?(Connector) #if var is connector return connector
    return var.out if var.is_a?(ArithmeticConstraint) #if var is ArithmeticConstraint return output
  end
