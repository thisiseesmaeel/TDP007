...
class ArithmeticConstraint
  ...
  def new_value(connector)
    ...
    #added so that we can check the value of a if the output is changed
    #sends inverse_op to object
    ...
    #added so that we can check the value of b if the output is changed
    #sends inverse_op to object
    if [out,a].include?(connector) and out.has_value? and a.has_value? and
...
def celsius2fahrenheit
  c = Connector.new("Celsius")
  f = Connector.new("Fahrenheit")
  a = ConstantConnector.new("a", 1.8)
  b = ConstantConnector.new("b", 32)
  d = Connector.new("d")
  Multiplier.new(c,a,d)
  Adder.new(d,b,f)
