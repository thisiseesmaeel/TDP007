...
class ConstraintParser < Parser
  ...
  def initialize
    ...
    @parser=super("constraints") do
      ...
      token(/[[:alpha:]]+/) do |connector|
        ...
        @connectors[connector] ||= Connector.new(connector) # a = b unless a (dvs a = b om inte a är definierad)
      end
      token(/\d+/) do |constant|
      ...
      rule :expr do
        ...
        match(:expr, '-', :term) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'-',b)
          # a-b=c <=> a=b+c
          Adder.new(conn_a,conn_b,conn_c).shift_operators
        end
      ...
      rule :term do
        ...
        match(:term, '/', :atom) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'/',b)
          # a/b=c <=> b*c=a
          Multiplier.new(conn_a,conn_b,conn_c).shift_operators
        end
  ...
  def get_connectors(conn_a,op,conn_b)
    ...
    puts "conn_a: #{conn_a}, conn_b: #{conn_b}, conn_c #{conn_c}"
    [conn_a,conn_b,conn_c]
  end

  def get_connector(con)
    if con.is_a? Connector
      puts "get_connector returns connector: #{con}"
      con
    elsif con.is_a? ArithmeticConstraint
      puts "get_connector returns ArithmeticConstraint: #{con.out}"
      con.out
    end
  end
  ...
  def replace_conn(lh,rh)
    ...
    if rh.is_a?(ArithmeticConstraint) then
      exp,conn=rh,lh_conn
      puts "EXP: #{exp}, CONN: #{conn} = RH: #{rh}, LH_CONN: #{lh_conn}"
      puts "EXP CLASS: #{exp.class}"
      puts "rh_conn.name: #{rh_conn.name} @@@@@@@@@@@@@@@@@@@@@@@@"
      @connectors.delete(rh_conn.name)
    else if lh.is_a?(ArithmeticConstraint) then
           exp,conn=lh,rh_conn
           puts "EXP: #{exp}, CONN: #{conn} = LH: #{lh}, RH_CONN: #{rh_conn}"
           puts "lh_conn.name: #{lh_conn.name} @@@@@@@@@@@@@@@@@@@@@@@@"
