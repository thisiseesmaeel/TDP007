  ...
  def new_value(connector)
    if [a,b,out].include?(connector)
      if a.has_value? and b.has_value? and (not out.has_value?) 
        ...
        puts "b = #{b}"
        # a + b = c
        val=a.value.send(op, b.value)   # SAMMA SAK SOM a.value.send(:+, b.value) dvs c = a + b
        logger.debug("#{self} : #{out} updated")
        out.assign(val, self)
      elsif a.has_value? and out.has_value? and (not b.has_value?) 
        puts "IN SECOND IF"
        # a + b = c
        # b = c - a
        #val=a.value.send(inverse_op, out.value)
        val=out.value.send(inverse_op, a.value)
        logger.debug("#{self} : #{b} updated")
        b.assign(val, self)
        
      elsif b.has_value? and out.has_value? and (not a.has_value?) 
        puts "IN THIRD IF"
...
class Connector
  ...
  def assign(v,setter)
      if not has_value? then
        ...
        @has_value=true
        @informant=setter
        (@constraints-[setter]).each { |c| c.new_value(self); puts "ASSIGN CONSTRAINTS LOOP" }
        "ok"
      else
        if value != v then
...
def celsius2fahrenheit
  ...
  c = Connector.new("c")
  result = Connector.new("=")
  sum = Connector.new("( )")
  f = Connector.new("f")

  const1 = ConstantConnector.new("9", 9)
  const2 = ConstantConnector.new("5", 5)
  const3 = ConstantConnector.new("32", 32)

  Multiplier.new(c, const1, result)
  Multiplier.new(sum, const2, result)
  Adder.new(sum, const3, f)
