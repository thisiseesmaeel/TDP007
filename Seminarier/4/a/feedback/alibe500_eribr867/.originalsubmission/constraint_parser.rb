...
class ConstraintParser < Parser
  #For testing purposes, can be removed otherwise.
  ...
  def initialize
    ...
    @parser=super("constraints") do
      ...
      rule :expr do
        ...
        match(:expr, '-', :expr) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'-',b)
          Subtractor.new(conn_a,conn_b,conn_c)
        end
  ...
  # Retrieve and generate connectors for the constraints
  def get_connectors(conn_a,op,conn_b)
    if conn_a.is_a?(ArithmeticConstraint)
      conn_a = conn_a.out
    end
    if conn_b.is_a?(ArithmeticConstraint)
      conn_b = conn_b.out
    end

    name_c="#{conn_a.name}#{op}#{conn_b.name}"
    if @connectors.keys.include?(name_c)
      return [conn_a,conn_b,@connectors[name_c]]
    else
  ...
  def get_connector(expr)
    if expr.is_a?(ArithmeticConstraint)
      expr = expr.out
    end
    name="#{expr}"

    if @connectors.keys.include?(name)
      return @connectors[name]
    else
      conn=Connector.new(name)
  ...
  def replace_conn(lh,rh)
    ...
    if rh.is_a?(ArithmeticConstraint) then
      ...
      #repurposed adder as = constraint
      zero_conn = Connector.new("0")
      @connectors["EQUALSFIX"] = zero_conn
      Adder.new(lh,zero_conn,rh)
      zero_conn.user_assign(0)
    end
  end
  ...
  def parse(str)
    @connectors={}
    ret = super(str)
    #removes zero_conn if it exists
    if @connectors.keys.include?("EQUALSFIX")
      ret = ret[0..-2]
      if ret.length == 1
        return ret[0]
      end
