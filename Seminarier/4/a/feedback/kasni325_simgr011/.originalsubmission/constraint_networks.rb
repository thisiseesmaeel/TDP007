...
class ArithmeticConstraint
  ...
  def new_value(connector)
    ...
    if [out,b].include?(connector) and out.has_value? and b.has_value? and 
      (not a.has_value?) then 
    # Inputs changed, so update output to be the sum of the inputs
    # "send" means that we send a message, inverse_op in this case, to an
    # object.
    val=out.value.send(inverse_op, b.value)
    logger.debug("#{self} : #{a} updated")
    a.assign(val, self)
    end
    if [a,out].include?(connector) and a.has_value? and out.has_value? and 
...
def celsius2fahrenheit
    ...
    constMult = Connector.new("constMult")
    constAdd = Connector.new("constAdd")
    constMult.user_assign(1.8)
    constAdd.user_assign(32)
    result = Connector.new("result")

    Multiplier.new(celsius, constMult, result)
    Adder.new(result, constAdd, fahrenheit)
