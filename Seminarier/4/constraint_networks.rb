...
def test_constraints
  ...
  # Logger::DEBUG
  c.log_level=Logger::DEBUG ## VAD INNEBÄR "C" HÄR?
  ...
  b.value=true
  gets
  a.value=false ## SKA DET VARA "C" DÅ?
  gets

  a=Wire.new('a')  ## VI HAR DE HÄR REDAN DAR UPPE.
  b=Wire.new('b')
  c=Wire.new('c')
...
class ArithmeticConstraint
  ...
  def new_value(connector)
    ...
    elsif (not a.has_value?) and b.has_value? and out.has_value? then
      val = out.value.send(inverse_op, b.value)
      logger.debug("#{self} : #{a} updated")
      a.assign(val, self)

    elsif (not b.has_value?) and a.has_value? and out.has_value? then
...
def celsius2fahrenheit
  ...
  num = ConstantConnector.new("32", 32)
  factor = ConstantConnector.new("9/5", 1.8)
  celsiusmultfactor = Connector.new("(C * 9/5)")
  
  Multiplier.new(celsius, factor, celsiusmultfactor)
  Adder.new(celsiusmultfactor, num, fahrenheit) 
