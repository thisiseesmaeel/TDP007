...
class ConstraintParser < Parser
    ...
    @parser=super("constraints") do
      ...
      start :statement do
        ...
        match(:expr, '=', :term) do |lh, _, rh|
          # happends in the case of c + 273 = k
          if lh.is_a?(Connector)
            lh = make_empty_adder(lh)
          end

          if rh.is_a?(Connector)
            rh = make_empty_adder(rh)
          end
      ...
      rule :expr do
        ...
        match(:expr, '-', :term) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'-',b)
          Subtracter.new(conn_a,conn_b,conn_c)
      ...
      rule :term do
        ...
        match(:term, '/', :atom) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'/',b)
          Divider.new(conn_a,conn_b,conn_c)
      ...
      rule :atom do
        match(Connector)
        match('(', :expr, ')') do |_, a, _| 
          if a.is_a?(ArithmeticConstraint)
            a.out
          else
            a
          end
  ...
  def make_empty_adder(connector)
    nothing = Connector.new("nothing", 0)
    conn_a, conn_b, conn_c = get_connectors(connector, "+", nothing)
    adder = Adder.new(conn_a, conn_b, conn_c)
    return adder
  end

  def receive_name(conn_a, op, conn_b)
    if conn_a.is_a?(Connector)
      name_one = "#{conn_a.name}"
    else
      name_one = "#{conn_a.out}"
    end

    if conn_b.is_a?(Connector)
      name_three = "#{conn_b.name}"
    else
      name_three = "#{conn_b.out}"
    end

    return "#{name_one}#{op}#{name_three}"
  end

  # Retrieve and generate connectors for the binary constraints 
  def get_connectors(conn_a, op, conn_b)

    name_c = receive_name(conn_a, op, conn_b)

    conn_c = Connector.new(name_c)
    @connectors[name_c] = conn_c
    @connectors.each { |n| puts n}
    [conn_a, conn_b, conn_c]
  end

  def get_connector(conn)
    if conn.is_a?(ArithmeticConstraint)
      return conn.out
    else
      return conn.name
    end
...
# e, m = cp.parse("e = (m * 299792458) * 299792458")
