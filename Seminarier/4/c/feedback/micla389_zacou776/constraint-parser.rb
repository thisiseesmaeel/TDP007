...
class ConstraintParser < Parser
    ...
    @parser=super("constraints") do
      ...
      rule :term do
        ...
        match(:term, '/', :term) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'/',b)
          # a/b=c <=> b*c=a
          Multiplier.new(conn_b,conn_c,conn_a)
        end
  ...
  def get_connectors(conn_a,op,conn_b)
    # Make sure conn_a and conn_b actually are connectors
    # in the event of nested operations
    conn_a = get_connector(conn_a) if !conn_a.is_a? Connector
    conn_b = get_connector(conn_b) if !conn_b.is_a? Connector
  ...
  # Return the "result" connector
  def get_connector(side)
    if side.is_a? Connector
      return side
    else
      # Select the connector with the longest name. This is ok
      # since the auto-generated connector will always have a longer name
      # than the other (see get_connectors())
      [side.a, side.b, side.out].max{|a,b| a.name.length <=> b.name.length}
    end
  end
  ...
  def replace_conn(lh,rh)
    ...
    if rh.is_a?(ArithmeticConstraint) then
      exp,conn=rh,lh_conn
      rh.a == rh_conn ? out_conn = "a" : out_conn = "b"
      out_conn = "out" if rh.out == rh_conn
      @connectors.delete(rh_conn.name)
    else
      if lh.is_a?(ArithmeticConstraint) then
        exp,conn=lh,rh_conn
        lh.a == lh_conn ? out_conn = "a" : out_conn = "b"
        out_conn = "out" if lh.out == lh_conn
        @connectors.delete(lh_conn.name)
      end
    end
    # Join lh and rh in the equality by connecting the constraint side
    # with the connector side
    eval("exp." + out_conn + "=conn") # :)
...
# D, [2007-03-03T15:16:56.458298 #18327] DEBUG -- : f got new value: 0 D, [2007-03-03T15:16:56.458455 #18327] DEBUG -- : f-32 + 32 == f : f-32 updated
