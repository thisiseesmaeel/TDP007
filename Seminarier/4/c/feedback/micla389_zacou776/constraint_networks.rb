...
class ArithmeticConstraint
  ...
  def new_value(connector)
    ...
    # "^" betyder XOR, ex: a=1 b=1, out=0 annars samma som OR
    elsif out == connector and !a.has_value? ^ !b.has_value? and
      out.has_value? then
      # Set input values when output changes
      with_value = a.has_value? ? a : b
      without_value = !a.has_value? ? a : b
      val=out.value.send(inverse_op, with_value.value)
      logger.debug("#{self} : #{out} updated")
      without_value.assign(val, self)
    end
    self
  end
...
def celsius2fahrenheit
  ...
  # 9c = 5(f - 32)
  leftfactor = Connector.new("factor left")
  leftfactor.user_assign(9)
  term = Connector.new("term")
  term.user_assign(32)
  rightfactor = Connector.new("factor right")
  rightfactor.user_assign(5)

  left = Connector.new("left")
  Multiplier.new(c, leftfactor, left)

  sum = Connector.new("sum")
  Adder.new(sum,term,f)

  Multiplier.new(sum, rightfactor, left)
  return c,f
end
