...
#OWN CODE: (Inspired by rbparse) 
class Compiler
  def initialize
    
    #Parsning
    @parser = Parser.new("Textbook") do
      #Lexer
      token(/\s+/)
      token(/\d+/){|m| m.to_i}
      token(/[\w\b]+/){|m| m} #(set varname 1)
      token(/./){|m| m}
  ...
  def code(code_str_arr,  prog_name="Parser code", i = 0)
    #print("[#{prog_name}: #{line}] ")
    if code_str_arr[i] == "exit" then
      puts "Exiting coding..."
    else
      return "#{@parser.parse code_str_arr[i]}"
      i += 1
      code code_str_arr, prog_name, line
    end
    

  end

  #In order to use Parser logger correctly, this function was copied
  def log(state = true)
    if state
