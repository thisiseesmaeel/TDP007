...
class ArithmeticConstraint
  ...
  def initialize(a, b, out)
    @logger=Logger.new(STDOUT)
    @a,@b,@out=[a,b,out]
    #@out = reorder_values(a,b,out)
    [a,b,out].each { |x| x.add_constraint(self) }
  end

  # def reorder_values(a,b,c)
  #   if c.has_value? and (not a.has_value? or not b.has_value?)
  #     new_out = [a,b,c].each { |val| value?(val) }
  #     @operator = inverse_op
  #     puts new_out
  #     return new_out
  #   else
  ...
  def new_value(connector)
    ...
    elsif [b,out].include?(connector) and out.has_value? and b.has_value? and 
    ...
    elsif [a,out].include?(connector) and out.has_value? and a.has_value? and 
...
def celsius2fahrenheit
  ...
  c = Connector.new("c")
  f = Connector.new("f")
  constant = ConstantConnector.new("const")
  factor = Connector.new("factor")
  product = ConstantConnector.new("product")
  constant.user_assign(32)
  factor.user_assign(1.8)
  Multiplier.new(c, factor, product)
  Adder.new(product,constant, f)
  return c,f
