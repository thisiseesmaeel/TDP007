...
class ConstraintParser < Parser
    ...
    @parser=super("constraints") do
      ...
      rule :expr do
        ...
        match(:expr, '-', :term) do |a, _, b|
          conn_a,conn_b,conn_c=get_connectors(a,'-',b)
          # a-b=c <=> a=b+c
          Subtracter.new(conn_a,conn_b,conn_c)#POS RAD
        end
  ...
  def get_connector(a)
    if a.is_a?(ArithmeticConstraint) then
      return @connectors[a.out.to_s]
    end
    return a
  end
...
ObjectSpace.each_object { |o|
  if o.is_a? ArithmeticConstraint
    puts "TYPE: #{o.class}  A : #{o.a}, B : #{o.b}, OUT : #{o.out}"
  end
}
