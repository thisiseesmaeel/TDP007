...
class ArithmeticConstraint
  ...
  def new_value(connector)
    ...
    elsif [out,b].include?(connector) and b.has_value? and out.has_value? and 
...
def celsius2fahrenheit
  #add_c är Farenheit, multi_d är celsius
  add_a = Connector.new("a")
  add_b = Connector.new("b")
  add_c = Connector.new("c")
  Adder.new(add_a, add_b, add_c)

  multi_d = Connector.new("d")
  multi_e = Connector.new("f") 

  Multiplier.new(multi_d, multi_e, add_b)

  add_a.user_assign(32)
  multi_e.user_assign(1.8)

  return multi_d, add_c
end
