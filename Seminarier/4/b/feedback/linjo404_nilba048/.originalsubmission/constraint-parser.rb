...
class ConstraintParser < Parser
  ...
  def get_connectors(conn_a,op,conn_b)
    ...
    # replace ArithmeticConstraint with ArithmeticConstraint.out
  ...
  def get_connector(v)
    if v.is_a?(Connector)
      return v
    elsif v.is_a?(ArithmeticConstraint)
      # find and return the result connector of the constraint
      array = [v.a.name, v.b.name, v.out.name]
      if array.max_by(&:length) == v.a.name
        return @connectors[v.a.name]
      elsif array.max_by(&:length) == v.b.name
        return @connectors[v.b.name]
      else
        return @connectors[v.out.name]
      end 
    end
  end
  ...
  def replace_conn(lh,rh)
    ...
    if rh.is_a?(ArithmeticConstraint) then
      exp, conn, other_conn = rh, lh_conn, rh_conn
      @connectors.delete(rh_conn.name)
    elsif lh.is_a?(ArithmeticConstraint) then
           exp, conn, other_conn = lh, rh_conn, lh_conn
           @connectors.delete(lh_conn.name)
    end

    # replace result connector in exp 
    if exp.a == other_conn
      exp.a=conn
    elsif exp.b == other_conn
      exp.b=conn
    else
      exp.out=conn
    end
