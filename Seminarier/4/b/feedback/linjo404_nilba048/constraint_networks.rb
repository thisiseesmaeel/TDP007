...
class ArithmeticConstraint
  ...
  def new_value(connector)
    # assign value to out when a and b is assigned
    if [a,b].include?(connector) and a.has_value? and b.has_value? and 
      ...
      out.assign(val, self)
    # assign value to b when a and out is assigned
    elsif [a,out].include?(connector) and a.has_value? and out.has_value? and 
      (not b.has_value?) then 
      val=out.value.send(inverse_op, a.value)
      logger.debug("#{self} : #{b} updated")
      b.assign(val, self)
    # assign value to a when b and out is assigned
    elsif [b,out].include?(connector) and b.has_value? and out.has_value? and 
...
def celsius2fahrenheit
  c1 = ConstantConnector.new("9", 9)
  c2 = ConstantConnector.new("-32", -32)
  c3 = ConstantConnector.new("5", 5)

  result1 = Connector.new("celsius*9")
  result2 = Connector.new("fahrenheit-32")

  celsius = Connector.new("celsius")
  fahrenheit = Connector.new("fahrenheit")

  Multiplier.new(celsius, c1, result1)
  Adder.new(fahrenheit, c2, result2)
  Multiplier.new(result2, c3, result1)
